import { Avatar, Button, Col, Descriptions, Form, Modal, Popconfirm, Row, Tag } from "antd";
...
import { approveAppPublishRequestService, getInprogressAppPublishRequestByAppIdService } from "../../services/app_publish_request_service";
import FormDenyAppPublishRequest from "./modalDenyPublishRequest";
import "./styles.css"
const ApplicationDetailPage = () => {
    const { appId } = useParams()
    const [app, setApp] = useState()
    const [inprogressPublish, setInprogressPublish] = useState()
    const [isDenyAppPublishModalVisible, setDenyAppPublishModalVisible] = useState(false)
    ...
    return (<>
        {app ? <>
            <div id="account-info-panel">
                <Col span={24}>
                    <Descriptions title="App Info">
                        <Descriptions.Item label="Name" labelStyle={{ fontWeight: "bold" }}>{app.name}</Descriptions.Item>
                        <Descriptions.Item label="Category" labelStyle={{ fontWeight: "bold" }}>{app.appCategoryName}</Descriptions.Item>
                        <Descriptions.Item label="Party" labelStyle={{ fontWeight: "bold" }}>{app.partyName}</Descriptions.Item>
                        <Descriptions.Item label="Party Email" labelStyle={{ fontWeight: "bold" }}>{app.partyEmail}</Descriptions.Item>
                        ...
                        <Row>
                            {
                                inprogressPublish ? <>
                                    <Popconfirm
                                        ...
                                        onConfirm={() => { approveAppPublishRequest() }}
                                        onCancel={() => { }}
                                        okText="Yes"
                                        cancelText="No"
                                    >
                                        <Button className="success-button" style={{ margin: 10 }}>
                                            Approve Publish
                                        </Button>
                                    </Popconfirm>
                                    <Popconfirm
                                        title="Are you sure to deny this app?"
                                        onConfirm={() => { setDenyAppPublishModalVisible(true) }}
                                        onCancel={() => { }}
                                        okText="Yes"
                                        cancelText="No"
                                    >
                                        <Button className="danger-button" style={{ margin: 10 }}>
                                            Deny Publish
                                        </Button>
                                    </Popconfirm>
                                </> : null
            ...
            : null}
        {
            inprogressPublish ? <Modal title="Deny Application Publish Request"
                visible={isDenyAppPublishModalVisible}
                onCancel={() => { setDenyAppPublishModalVisible(false) }}
                footer={null}>
                <FormDenyAppPublishRequest onSuccess={onDenySuccess} requestId={inprogressPublish.id} />
            </Modal> : null
        }
