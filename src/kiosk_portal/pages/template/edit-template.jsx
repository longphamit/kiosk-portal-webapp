...
import {
  getListAvailableCategoriesService,
  getListAvailableEventsService,
} from "../../services/categories_service";
import { Option } from "antd/lib/mentions";
import { useNavigate, useSearchParams } from "react-router-dom";
import {
  createAppCategoryPosition,
  createEventPosition,
  getAppCategoryPositionService,
  getEventPositionService,
  getTemplateById,
  updateAppCategoryPosition,
  updateEventPosition,
} from "../../services/template_service";
import {
  TEMPLATE_CREATE_LABEL,
  TEMPLATE_EDITING_HREF,
  TEMPLATE_MANAGER_HREF,
  TEMPLATE_MANAGER_LABEL,
} from "../../components/breadcumb/breadcumb_constant";
import CustomBreadCumb from "../../components/breadcumb/breadcumb";
import {
  buildPositionsModelRequest,
  createEventModel,
...
const EditTemplatePage = () => {
      ...
      if (listAppCategoryPosition.length !== 0) {
        for (let p of listAppCategoryPosition) {
          if (p.components.length !== 0) {
            let dataTemp = [];
            for (let component of p.components) {
              let appCategory = getComponentFromList(
                component.appCategoryId,
                allCategories
              );
              allCategories = removeItemFromList(
                component.appCategoryId,
                allCategories
              ); // set available row
              dataTemp[component.columnIndex] = {
                id: appCategory.id,
                image: appCategory.image,
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success(
        selectedType === SELECTED_TYPE_CATEGORY
          ? "Save category components successfully"
          : "Save event components successfully"
      );
    } catch (e) {
      console.error(e);
          ...
          >
            <Row>
              <Col span={8}>
                ...
                >
                  <Option
                    value={SELECTED_TYPE_CATEGORY}
                    style={{ textTransform: "capitalize" }}
                  >
                    {SELECTED_TYPE_CATEGORY}
                  </Option>
                  <Option
                    value={SELECTED_TYPE_EVENT}
                    style={{ textTransform: "capitalize" }}
                  >
                    {SELECTED_TYPE_EVENT}
                  </Option>
